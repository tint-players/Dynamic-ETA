...
function routeIndicatorLit(config: SimulatorConfigViz, frames: TelemetryFrame[], signalId: string, aspect: SignalAspect): boolean {
  const signal = config.signals.find((item) => item.signal_id === signalId)
  if (!signal || signal.signal_type !== 'ROUTE_INDICATOR' || !signal.crossover_id || aspect !== 'GREEN') return false
  const crossover = config.crossovers.find((item) => item.crossover_id === signal.crossover_id)
  if (!crossover || signal.track_id !== crossover.from_track_id || signal.direction !== 'FORWARD') return false
...
export default function RailwayRouteV2({ config, frames, signalStates, crossingStates, selectedTrainId, onSelectTrain, constraintsOpen = false, activeConstraintTool = 'weather', rangeDraft = null, onRangeDraft, onRangeDraftChange, onSignalSelect, selectedSignalId = '', manualSignalOverrides = {}, constraintState }: {
  ...
  return <section className={`panel route-panel realistic-route-panel route-focus ${constraintsOpen ? 'constraint-editing' : ''}`}>
    ...
    <div className="realistic-route-scroll" ref={scrollRef}><svg ref={svgRef} className="realistic-route" style={{ width: `${WIDTH * zoom}px` }} viewBox={`0 0 ${WIDTH} ${HEIGHT}`} onPointerMove={(event) => dragRange(event.clientX)} onPointerUp={() => { dragHandle.current = null }} onPointerLeave={() => { dragHandle.current = null }}>
      ...
      {frames.map((frame, index) => { const reverse = frame.direction === 'REVERSE'; const directionSign = reverse ? -1 : 1; const selected = frame.train_id === selectedTrainId; const run = config.trains.find((item) => item.train.train_id === frame.train_id); const trainLengthM = run?.train.length_m ?? config.train.length_m; const dualCab = run?.train.dual_cab ?? false; const compartmentCount = trainCompartmentCount(trainLengthM); const compartmentLengthM = trainLengthM / compartmentCount; const coachWidthPx = Math.max(26, metersToPixels(config, compartmentLengthM) - COACH_GAP_PX); const frontPose = displayPoseAt(config, frame, frame.route_position_m); const labelX = snapHalf(frontPose.x); const labelY = snapHalf(frontPose.y - 38); return <g key={frame.train_id} className={`svg-train-position train-${index % 4} ${frame.active ? 'active' : 'waiting'} ${frame.completed ? 'completed' : ''} ${selected ? 'selected' : ''}`} onClick={() => onSelectTrain(frame.train_id)}>{Array.from({ length: compartmentCount }, (_, compartmentIndex) => { const distanceBehindFrontM = (compartmentIndex + 0.5) * compartmentLengthM; const coachRoutePositionM = frame.route_position_m - directionSign * distanceBehindFrontM; const coachPose = displayPoseAt(config, frame, coachRoutePositionM); const isFront = compartmentIndex === 0; const isRear = compartmentIndex === compartmentCount - 1; const x = -coachWidthPx / 2; return <g key={compartmentIndex} transform={`translate(${coachPose.x} ${coachPose.y}) rotate(${coachPose.angleDeg}) ${reverse ? 'scale(-1 1)' : ''}`} className="svg-train-body train-compartment"><rect x={x} y="-12" width={coachWidthPx} height="20" rx="5" className="train-coach-shell" /><rect x={x + coachWidthPx * .2} y="-8" width={Math.max(5, coachWidthPx * .17)} height="6" rx="1.5" className="train-window" /><rect x={x + coachWidthPx * .5} y="-8" width={Math.max(5, coachWidthPx * .17)} height="6" rx="1.5" className="train-window" />{isFront && <path d={`M ${coachWidthPx / 2 - 9} -12 L ${coachWidthPx / 2} -6 L ${coachWidthPx / 2} 5 L ${coachWidthPx / 2 - 9} 8 Z`} className="train-cab" />}{dualCab && isRear && <path d={`M ${x + 9} -12 L ${x} -6 L ${x} 5 L ${x + 9} 8 Z`} className="train-cab" />}<circle cx={x + 9} cy="10" r="3" /><circle cx={x + coachWidthPx - 9} cy="10" r="3" /></g> })}<g transform={`translate(${labelX} ${labelY})`} className="train-label-anchor"><rect x="-72" y="-12" width="144" height="20" rx="8" className="train-label-bg" /><text x="0" y="-1" textAnchor="middle" dominantBaseline="middle" className="train-speed-label">{frame.train_id} · {Math.round(frame.speed_kmh)}</text></g></g> })}
