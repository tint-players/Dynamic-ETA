...
function ConstraintDrawer({ config, simTime, playback, activeTool, onTool, selectedSignalId, onSelectedSignal, restrictionTrack, onRestrictionTrack, rangeDraft, onRangeDraft, onInject }: {
  ...
  return <aside className="constraints-drawer panel">
    ...
    {(activeTool === 'tsr' || activeTool === 'maintenance') && <div className="drawer-section"><div className="drawer-section-title"><strong>{activeTool === 'tsr' ? 'Temporary speed restriction' : 'Maintenance'}</strong><span>click either track or use the menu</span></div><label className="drawer-field">Track<select value={restrictionTrack} onChange={(event) => onRestrictionTrack(event.target.value)}>{config.route.track_ids.map((trackId) => <option key={trackId} value={trackId}>{trackId}</option>)}</select></label>{activeTool === 'maintenance' && <label className="drawer-field">Mode<select value={maintenanceType} onChange={(event) => setMaintenanceType(event.target.value as MaintenanceType)}><option value="SPEED_RESTRICTION">SPEED RESTRICTION</option><option value="FULL_CLOSURE">FULL CLOSURE</option></select></label>}<div className={`range-readout ${rangeDraft?.kind === activeTool ? 'ready' : ''}`}>{rangeDraft?.kind === activeTool ? <><strong>{rangeDraft.track_id} · {rangeDraft.block_id}</strong><span>{Math.round(rangeDraft.start_position_m)}m → {Math.round(rangeDraft.end_position_m)}m</span><small>{Math.round(rangeDraft.end_position_m - rangeDraft.start_position_m)}m selected · drag either handle on that track</small></> : <><strong>No range selected</strong><span>Click directly on TRACK-UP or TRACK-DOWN</span><small>The clicked track becomes selected and the menu updates automatically.</small></>}</div>{rangeDraft?.kind === activeTool && <button type="button" className="drawer-remove" onClick={() => onRangeDraft(null)}>Remove selection</button>}{(activeTool === 'tsr' || maintenanceType === 'SPEED_RESTRICTION') && <Stepper label="Speed limit" value={restrictionLimit} min={10} max={160} step={5} suffix=" km/h" onChange={setRestrictionLimit} />}<Stepper label="Duration" value={restrictionDuration} min={10} max={3600} step={10} suffix=" s" onChange={setRestrictionDuration} /><button className="drawer-apply" disabled={disabled || !restrictionTrack || !rangeDraft || rangeDraft.kind !== activeTool || rangeDraft.track_id !== restrictionTrack} onClick={applyRestriction}>Apply {activeTool === 'tsr' ? 'TSR' : maintenanceType === 'FULL_CLOSURE' ? 'full closure' : 'maintenance'}</button></div>}
    ...
    <div className="drawer-tip"><strong>Direct manipulation</strong><span>TSR/Maintenance: click whichever physical track you want. That track is selected automatically, the menu follows it, and a 200m range is created there. Drag either edge to resize.</span></div>
  </aside>
}
