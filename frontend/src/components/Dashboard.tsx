  ...
  return (
    <section className="controls panel">
      <div className="transport-buttons">
        <button onClick={playback.playing ? onPause : onPlay} className="primary">{playback.playing ? 'Pause' : 'Play'}</button>
        <button onClick={onStep} disabled={playback.playing || playback.complete}>Step</button>
        <button onClick={onReset}>Reset</button>
      </div>
      <div className="speed-buttons"><span>Playback</span>{speeds.map((speed) => <button key={speed} className={playback.playback_speed === speed ? 'selected' : ''} onClick={() => onSpeed(speed)}>{speed}×</button>)}</div>
...
function FleetPanel({ frames }: { frames: TelemetryFrame[] }) {
  return (
    <section className="panel fleet-panel">
      <div className="panel-heading"><h2>Live fleet</h2><span>{frames.length} trains</span></div>
      <div className="fleet-grid">
        {frames.map((train) => (
          <div className={`fleet-card ${train.active ? 'active' : ''} ${train.completed ? 'completed' : ''}`} key={train.train_id}>
...
function TrainStatePanel({ frame }: { frame: TelemetryFrame }) {
  return (
    <section className="panel">
      <div className="panel-heading"><h2>Primary train state</h2><span className="action-badge">{frame.control_action}</span></div>
      <div className="metric-grid">
        <Metric label="Simulation time" value={`${fmt(frame.sim_time_s, 0)} s`} />
        <Metric label="Track / direction" value={`${frame.track_id} · ${frame.direction}`} />
        ...
        <Metric label="Station" value={frame.current_station_id ?? '—'} />
      ...
      <div className="ground-truth-note">Ground Truth Remaining Time: {frame.actual_remaining_time_s == null ? 'unavailable during live run' : `${fmt(frame.actual_remaining_time_s, 0)} s`}</div>
    </section>
  )
}
...
function ConstraintPanel({ frame }: { frame: TelemetryFrame }) {
  ...
  return <section className="panel"><div className="panel-heading"><h2>Controller lookahead</h2></div><div className="constraint-list">{rows.map(([name, value, distance]) => <div className="constraint-row" key={name}><span>{name}</span><strong>{value}</strong><small>{distance == null ? '—' : `${fmt(distance, 0)} m ahead`}</small></div>)}</div></section>
}

function SpeedChart({ history }: { history: TelemetryFrame[] }) {
  const data = history.slice(-240)
  const width = 900, height = 230, pad = 30
  ...
  return <section className="panel chart-panel"><div className="panel-heading"><h2>Primary speed vs effective ceiling</h2><span className="chart-key"><i className="actual-line" />Actual <i className="ceiling-line" />Ceiling</span></div><svg className="speed-chart" viewBox={`0 0 ${width} ${height}`}><line x1={pad} y1={height-pad} x2={width-pad} y2={height-pad} className="axis"/><line x1={pad} y1={pad} x2={pad} y2={height-pad} className="axis"/><polyline points={data.map((f) => point(f, f.effective_speed_ceiling_kmh)).join(' ')} className="ceiling-polyline"/><polyline points={data.map((f) => point(f, f.speed_kmh)).join(' ')} className="speed-polyline"/></svg></section>
}
...
export default function Dashboard(props: DashboardProps) {
  const { config, frame, frames, signalStates, history, events, playback, exportPaths } = props
  return (
    <main className="dashboard">
      <header className="topbar">
        <div><span className="eyebrow">Component A · multi-train network simulation</span><h1>Dynamic-ETA Railway Simulator</h1></div>
        <div className="scenario-card"><span>{frame.scenario_id}</span><strong>{frames.length} live trains · {config.stations.length} stations</strong><small>{config.dynamic_signalling ? 'occupancy-driven signalling' : 'scheduled signalling'}</small></div>
      </header>
      <SimulationControls {...props} />
      <RailwayRouteV2 config={config} frames={frames} signalStates={signalStates} />
      <FleetPanel frames={frames} />
      <div className="two-column"><TrainStatePanel frame={frame} /><ConstraintPanel frame={frame} /></div>
