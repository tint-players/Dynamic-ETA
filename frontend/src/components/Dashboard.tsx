...
export interface RangeDraft { kind: 'tsr' | 'maintenance'; block_id: string; start_position_m: number; end_position_m: number }
...
function ConstraintDrawer({ config, simTime, playback, activeTool, onTool, selectedSignalId, onSelectedSignal, rangeDraft, onRangeDraft, onInject }: {
  config: SimulatorConfigViz
  simTime: number
  playback: PlaybackState
  ...
  const applyRestriction = () => {
    ...
    if (rangeDraft.kind === 'tsr') {
      onInject({ command: 'inject_tsr', block_id: rangeDraft.block_id, start_position_m: rangeDraft.start_position_m, end_position_m: rangeDraft.end_position_m, speed_limit_kmh: restrictionLimit, duration_s: restrictionDuration })
    } else {
      onInject({ command: 'inject_maintenance', block_id: rangeDraft.block_id, start_position_m: rangeDraft.start_position_m, end_position_m: rangeDraft.end_position_m, speed_limit_kmh: restrictionLimit, maintenance_type: maintenanceType, duration_s: restrictionDuration })
    }
    onRangeDraft(null)
  }
  ...
  return <aside className="constraints-drawer panel">
    ...
    {(activeTool === 'tsr' || activeTool === 'maintenance') && <div className="drawer-section"><div className="drawer-section-title"><strong>{activeTool === 'tsr' ? 'Temporary speed restriction' : 'Maintenance'}</strong><span>click track to place</span></div>{activeTool === 'maintenance' && <label className="drawer-field">Mode<select value={maintenanceType} onChange={(event) => setMaintenanceType(event.target.value as MaintenanceType)}><option value="SPEED_RESTRICTION">SPEED RESTRICTION</option><option value="FULL_CLOSURE">FULL CLOSURE</option></select></label>}<div className={`range-readout ${rangeDraft?.kind === activeTool ? 'ready' : ''}`}>{rangeDraft?.kind === activeTool ? <><strong>{rangeDraft.block_id}</strong><span>{Math.round(rangeDraft.start_position_m)}m → {Math.round(rangeDraft.end_position_m)}m</span><small>{Math.round(rangeDraft.end_position_m - rangeDraft.start_position_m)}m selected · drag either handle on the network</small></> : <><strong>No range selected</strong><span>Click the live railway track</span><small>A centered 200m section will be created automatically.</small></>}</div>{rangeDraft?.kind === activeTool && <button type="button" className="drawer-remove" onClick={() => onRangeDraft(null)}>Remove selection</button>}{(activeTool === 'tsr' || maintenanceType === 'SPEED_RESTRICTION') && <Stepper label="Speed limit" value={restrictionLimit} min={10} max={160} step={5} suffix=" km/h" onChange={setRestrictionLimit} />}<Stepper label="Duration" value={restrictionDuration} min={10} max={3600} step={10} suffix=" s" onChange={setRestrictionDuration} /><button className="drawer-apply" disabled={disabled || !rangeDraft || rangeDraft.kind !== activeTool} onClick={applyRestriction}>Apply {activeTool === 'tsr' ? 'TSR' : maintenanceType === 'FULL_CLOSURE' ? 'full closure' : 'maintenance'}</button></div>}
    ...
    <div className="drawer-tip"><strong>Direct manipulation</strong><span>TSR/Maintenance: click the track for a 200m range, then drag either edge. Full closure stops trains before the selected maintenance zone. Signal: click the signal, then choose YELLOW or RED; reset returns it to automatic signalling.</span></div>
  </aside>
}
